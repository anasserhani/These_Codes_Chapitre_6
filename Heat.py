...
# -*- coding: utf-8 -*-
"""
Authors   : A. Serhani & G. Haine & D. Matignon & X. Vasseur
...
class Heat_2D:
    ...
    def __call__(self):
        if self.verbose: print('Heat class of Scrimp')
...
class Lyapunov_3D(Lyapunov_2D):
    #%% PROBLEM DEFINITION
    ## Method.
    # @param self The object pointer.
    def Set_Cubic_Domain(self, x0, xL, y0, yL, z0, zL):
        """
        Set the dimensions of the rectangular domain
        """
        self.x0 = x0
        ...
        if self.verbose: print(40*'-')
        if self.verbose: print('Cubic domain: OK')
        if self.verbose: print(40*'-')
    ...
    ## Method.
    #  @param self The object pointer.
    def Set_Physical_Parameters(self, rho, Lambda11, Lambda12, Lambda13, Lambda22,\
                                Lambda23, Lambda33, CV, **kwargs):
        """
        Set the physical parameters as a FeniCS expression related to the PDE
        """
        assert self.set_rectangular_domain == 1, 'The domain must be defined before.'
        ...
        self.rho        = Expression(rho, degree=2,\
                              x0=self.x0, xL=self.xL, y0=self.y0, yL=self.yL,\
                              z0=self.z0, zL=self.zL, **kwargs)
        self.Lambda     = Expression(( (Lambda11, Lambda12, Lambda13),\
                                       (Lambda12, Lambda22, Lambda23),\
                                       (Lambda13, Lambda23, Lambda33)),\
                                        degree=2, x0=self.x0, xL=self.xL,\
                                        y0=self.y0, yL=self.yL, z0=self.z0, zL=self.zL,\
                                        **kwargs)
        self.CV         = Expression(CV, degree=2,\
                              x0=self.x0, xL=self.xL, y0=self.y0, yL=self.yL,\
                              z0=self.z0, zL=self.zL, **kwargs)
    ...
    def Assign_Mesh(self, Msh):
        ...
        self.xv = Msh.coordinates()[:,0]
        self.yv = Msh.coordinates()[:,1]
        self.zv = Msh.coordinates()[:,2]
        self.Nv = self.Msh.num_vertices()

        # Domain limits
        self.x0, self.xL = self.xv.min(), self.xv.max() 
        self.y0, self.yL = self.yv.min(), self.yv.max()
        self.z0, self.zL = self.zv.min(), self.zv.max()
    ...
    def Generate_Mesh(self, rfn, structured_mesh=False):
        ...
        rfny     =  int (self.rfn * (self.yL - self.y0) / (self.xL - self.x0) ) 
        rfnz     =  int (self.rfn * (self.zL - self.z0) / (self.xL - self.x0) ) 

        
        if structured_mesh: 
            self.Msh = BoxMesh(Point(self.x0,self.y0,self.z0), Point(self.xL,self.yL,self.zL),\
                                     self.rfn, rfny, rfnz)
        else:
            self.Msh = generate_mesh(Box(Point(self.x0,self.y0,self.z0), Point(self.xL,self.yL,self.zL)),\
                                     self.rfn)
        
        # Explicit coordinates
        self.xv = self.Msh.coordinates()[:,0]
        self.yv = self.Msh.coordinates()[:,1]
        self.xv = self.Msh.coordinates()[:,2]
        self.Nv = self.Msh.num_vertices()
        
        self.norext   = FacetNormal(self.Msh)
        
        if self.verbose: print(40*'-')
        if self.verbose: print('Mesh:', 'Nv=', self.Msh.num_vertices())
        if self.verbose: print('Mesh: OK')
        if self.verbose: print(40*'-')
    ...
    def Save_for_Paraview(self, eT, eQ, interval=None):
        if interval is None:
            interval = range(Nt+1)
        ...
        eT_func = Function(self.Vs)
        eQ_func = Function(self.VV)

        vtkfile = File('para/eT.pvd')
        for i in interval:
            eT_func.vector()[range(self.Ns)] = eT[range(self.Ns), i]
            vtkfile << (eT_func, self.tspan[i])

        vtkfile = File('para/eQ.pvd')
        for i in interval:
            eQ_func.vector()[range(self.NV)] = eQ[range(self.NV), i]
            eQ_func = project(eQ_func, self.VV)
            vtkfile << (eQ_func, self.tspan[i])
...
#    E.Plot_Hamiltonian(tspan, Ham)
    ...
    #%% LYAPUNOV 3D
    ### Construction of 3D Lyapunov class
    Lya3D = Lyapunov_3D()
    
    ### Cubic domain
    x1_0, x1_ell, x2_0, x2_ell, x3_0, x3_ell = 0., 3., 0., 2., 0., 1,
    Lya3D.Set_Cubic_Domain(x1_0, x1_ell, x2_0, x2_ell, x3_0, x3_ell);
    
    ### Time
    t_initial, t_final  = 0., 2.
    Lya3D.Set_Initial_Final_Time(t_initial, t_final);

    ### Physical parameters
    rho    =  'x[0]*x[0] + x[1]*x[1] - x[2]*x[2] + 5' 
    ...
    Lya3D.Set_Physical_Parameters(rho, Lambda11, Lambda12, Lambda13, Lambda22,\
                                Lambda23, Lambda33, CV)
    
    ### Boundary control
    Ub_sp0  = ''' 
                + ( abs(x[0]) <= DOLFIN_EPS ? - x[1] * (Ly-x[1]) : 0 )            
                + ( abs(Lx - x[0]) <= DOLFIN_EPS ? exp(x[1] * (Ly-x[1])) -1  : 0 )      
                + ( abs(x[1]) <= DOLFIN_EPS ?  - x[0] * (Lx-x[0])/3  : 0 )            
                + ( abs(Ly - x[1]) <= DOLFIN_EPS ?  pow(x[0],2) * (Lx-x[0])/4  : 0 )     
            '''
    Ub_tm0  = lambda t : sin(4 * 2*pi/t_final * t) * 5e3 #  t * (tfinal - t)**4 * 25000
    Lya3D.Set_Boundary_Control(Ub_tm0=Ub_tm0, Ub_sp0=Ub_sp0, 
                          Ub_tm1=lambda t : 0, Ub_sp1='0', Lx=Constant(x1_ell), Ly=Constant(x2_ell))
    
    ### Initial data
    Lya3D.Set_Initial_Data(eT_0='0')

    ### Mesh
    rfn = 3
    xL, yL, zL = x1_ell, x2_ell, x2_ell
    bx = Box(Point(0,0,0), Point(xL,yL,zL)) #- Sphere(Point(xL/2, yL/2, zL/2), .25)
    cyl1 = Cylinder(Point(xL/2,yL/2,zL), Point(xL/2, yL/2, 0), .25, .25)
    cyl2 = Cylinder(Point(xL/2,0,zL/2), Point(xL/2, yL, zL/2), .25, .25)
    cyl3 = Cylinder(Point(0,yL/2,zL/2), Point(xL, yL/2, zL/2), .25, .25)
    Msh = generate_mesh(bx  - cyl1 - cyl2 - cyl3, 3) 
    
    Lya3D.Assign_Mesh(Msh)
    
    ### Finite elements families
    Lya3D.Set_Finite_Element_Spaces(family_scalar='P', family_Vector='RT', family_boundary='P', rs=1, rV=0, rb=1);
    
    ### Assembly
    Lya3D.Assembly()
    
    ### Discretization formulation
    Lya3D.Set_Formulation('div')
    
    ### Projections
    Lya3D.Project_Initial_Data()
    Lya3D.Project_Boundary_Control()
    
    ### Time setting
    dt     = 5.e-4
    Lya3D.Set_Time_Setting(dt)
    
    ### Time-stepping
    eT, eQ, Lyapunov = Lya3D.Integration_ODE_RK4()
    
    ### Plot Hamiltonian
    plt.close('all')
    Lya3D.Plot_Hamiltonian(Lya3D.tspan, Lyapunov, linewidth=3)
    
    ### Save solution
    interval = []
    for i in range(Lya3D.Nt+1):
        if i % 2 == 0:
            interval.append(i)
    Lya3D.Save_for_Paraview(eT,eQ, interval)
