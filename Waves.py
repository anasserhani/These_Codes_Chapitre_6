...
class Wave_2D:
    ...
    def Set_Mixed_BC_Dirichlet(self, Ub_tm0, Ub_sp0, Ub_tm1=lambda t : 0, Ub_sp1='0',\
        ...
        if self.verbose: print(40*'-')
        if self.verbose: print('Dirichlet boundary control: OK')
        if self.verbose: print(40*'-')
    ...
    def Set_Mixed_BC_Normal(self, Ub_tm0, Ub_sp0, Ub_tm1=lambda t : 0, Ub_sp1='0', **kwargs):
        ...
        self.Ub_sp1_N_Expression = Expression(Ub_sp1, degree=2,\
                                              ...
                                              **kwargs)
        if self.verbose: print(40*'-')
        if self.verbose: print('Normal boundary control: OK')
        if self.verbose: print(40*'-')
...
#
    ...
    def Plot_Mesh(self, figure=True, figsize=(8,6.5), save=False, **kwargs):
        """
    ...
    ## Method.
    #  @param self The object pointer.
    def Plot_Mesh_with_DOFs_MBC(self, figure=True, figsize=(8,6.5), **kwargs):
        """
        Plot the two-dimensional mesh with the FEniCS plot method including DOFs of the MBC problem
        """
        assert self.generate_mesh == 1, "The finite element mesh must be generated first"
        plot(self.Msh, linewidth=1, title='Les degrés de liberté sur la frontière: $N_\partial^{Dir}=$'+str(self.Nb_D)+', $N_\partial^{Nor}=$'+str(self.Nb_N)+ ' et $N_\partial^{Imp}=$'+str(self.Nb_Z))
        plt.plot(self.coord_b_full[self.D_index,0], self.coord_b_full[self.D_index,1], '.', markeredgewidth=2, label='$\Gamma_{Dir}$')
        plt.plot(self.coord_b_full[self.N_index,0], self.coord_b_full[self.N_index,1], '+', markeredgewidth=2, label='$\Gamma_{Nor}$')
        if 'impedance_mbc' in self.damp:
            plt.plot(self.coord_b_full[self.Z_index,0], self.coord_b_full[self.Z_index,1], 'x', markeredgewidth=2, label='$\Gamma_{Imp}$')
        plt.legend()
        
        if figure : plt.figure(figsize=figsize)
        if not(self.notebook):
            plt.show()
        plt.savefig("Mesh_with_DOFs_MBC.png")
    ...
    ## Method.
    #  @param self The object pointer.   
    def Quiver(self, time_space_Vector, t=0, with_mesh=False, title='', save=False, figsize=(8,6.5), **kwargs):
        """
        Create a quiver plot at a specific time t
        """
        ...
        Vec = Function(self.Vq)
        if with_mesh: 
            plot(self.Msh, linewidth=0.25)
        
        Vec.vector()[range(self.Nq)] = time_space_Vector[range(self.Nq), t] 
        plot(Vec, **kwargs)
...
#            except :    pass
    ...
    ## Method.
    # @param self The object pointer.
    def Trisurf(self, SpaceTimeVector_Lag, t, title='', save=False, figsize=(8,6.5), **kwargs):
        """
        Create a trisurf plot at a specific time t
        """
        t   = int(np.where(self.tspan == t)[0])
        fig = plt.figure(figsize=figsize)
        ax  = fig.gca(projection='3d')
        ...
        ax.set_ylim(self.y0, self.yL)
        ax.set_zlim(np.min(SpaceTimeVector_Lag), np.max(SpaceTimeVector_Lag))
        ax.plot_trisurf(self.xp, self.yp, SpaceTimeVector_Lag[:,t], linewidth=0.2, \
